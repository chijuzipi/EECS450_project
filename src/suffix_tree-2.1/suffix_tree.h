...
/* builds a suffix tree for the string s -- s must NOT contain the
   special UCharacter `term' (term ensures that all suffixes are leaves) */
suffix_tree_t *       st_make(const UChar *s, const UChar *term);
const node_t *        st_find(const suffix_tree_t *tree, const UChar *s);
