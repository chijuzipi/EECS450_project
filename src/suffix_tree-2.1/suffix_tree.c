...
suffix_tree_t *
make_helper(UChar *s, int length)
{
...
suffix_tree_t *
st_make(const UChar *s, const UChar *term)
{
    const UChar *c;
    for (c = s; *c; ++c) 
...
const node_t *
st_find(const suffix_tree_t *tree, const UChar *s)
{
    return find_helper(tree, tree->root, s, u_strlen(s));
}
