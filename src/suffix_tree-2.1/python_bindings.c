...
SuffixTree_string(SuffixTreeObject *self)
{
    ...
    str = (char *)malloc(sizeof(char) * (self->tree->str_len + 2));
    u_strToUTF8(str, self->tree->str_len + 2, NULL, self->tree->str, -1, &err);
    uniStr = PyUnicode_FromString(str);
    free(str);
...
Node_edge_label(NodeObject *self)
{
    ...
    /* Convert the UChar string to UTF8
     * and create the unicode object*/
    str = (char *)malloc(sizeof(char) * (length + 2));
    u_strToUTF8(str, length + 2, NULL, self->tree->tree->str + start, length, &err);
    uniStr = PyUnicode_FromString(str);
    free(str);
